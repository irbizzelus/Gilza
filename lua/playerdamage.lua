-- added new roll with advantage skill for dodge for burglar
Hooks:OverrideFunction(PlayerDamage, "damage_bullet", function (self, attack_data)
	...
	if dodge_roll < dodge_value then
		...
		if managers.player:has_category_upgrade("temporary", "dodge_roll_with_advantage") then
			if not managers.player:has_activate_temporary_upgrade("temporary", "dodge_roll_with_advantage") then
				managers.player:activate_temporary_upgrade("temporary", "dodge_roll_with_advantage")
