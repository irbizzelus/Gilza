...
local first_gun_rays = {}

-- override akimbo raycast to use a slighlty altered one. based on weaponlib's standard raycast.
-- the only change is that bullet fired from the second gun will land in the same spot as the bullet from the first shot
Hooks:OverrideFunction(AkimboWeaponBase, "_fire_raycast", function (self, user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, shoot_through_data, ammo_usage)
	...
	for i = 1, rays or 1 do
		...
		if self._second_gun then -- second gun is the one on the left and is the one that is fired first. makes sense i guess, but a bit weird
			table.insert(first_gun_rays, {mvec_to = Vector3(mvec_to.x, mvec_to.y, mvec_to.z)})
		else
			if first_gun_rays and first_gun_rays[i] then
				mvec_to = first_gun_rays[i].mvec_to
			end
		end
...
Hooks:OverrideFunction(AkimboWeaponBase, "fire", function (self, ...)
...
Hooks:OverrideFunction(AkimboWeaponBase, "_fire_second", function (self, params)
