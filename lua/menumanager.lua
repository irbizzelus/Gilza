...
function Gilza:changelog_message()
	DelayedCalls:Add("Gilza_showchangelogmsg_delayed", 1, function()
		if not Gilza.customguns.version or Gilza.customguns.version < 1.531 then
			local menu_options = {}
			menu_options[#menu_options+1] ={text = "Check full changelog", data = nil, callback = Gilza_linkchangelog}
			menu_options[#menu_options+1] = {text = "Cancel", is_cancel_button = true}
			local message = "1.5.31 update changelog:\n- Removed snowball adjustments to avoid issues after update 232.4\n\nProbably not the last fix for U232.4"
			...
			Gilza.customguns.version = 1.531
			Gilza.Save_gunz()
		end
	end)
end
